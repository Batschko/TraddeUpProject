...
DROP TABLE IF EXISTS `v_fullcs2skin_csmoney`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `v_fullcs2skin_csmoney` AS SELECT `t3`.`id` AS `id`,`t1`.`stash_id` AS `stash_id`,`t1`.`collection_id` AS `collection_id`,`t3`.`modified_date` AS `modified_date`,`t2`.`coll_name` AS `coll_name`,`t1`.`rarity` AS `rarity`,`t3`.`stattrak` AS `stattrak`,`t1`.`weapon` AS `weapon`,`t1`.`title` AS `title`,`t3`.`condition` AS `condition`,`t1`.`float_start` AS `float_start`,`t1`.`float_end` AS `float_end`,`t1`.`image_url` AS `image_url`,`t1`.`top` AS `top`,`t3`.`skin_price_id` AS `skin_price_id`,`t4`.`price` AS `price`,`t4`.`amount` AS `amount`
